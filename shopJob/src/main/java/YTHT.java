...
public class YTHT implements Tool {
	 ...
	 *     /to/merge/root/dir1/dir2
	 */
	private static void mergeDir(Path root, Path input, Path backupRoot,
		...
		try {
			if(isSeqFile) {
				mergeSequenceFile(fs, input, fs, new Path(output, testFileName), 
						conf, reporter);
			} else {
				CompressionCodecFactory compressionCodecs = new CompressionCodecFactory(conf);
				final CompressionCodec codec = compressionCodecs.getCodec(testFile);
				if (codec == null) {
					mergeNormalFile(fs, input, fs, new Path(output, testFileName), 
						conf, reporter);
				} else {
					mergeCompressedFile(fs, input, fs, new Path(output, testFileName), 
							conf, codec, reporter);
				}
			}
		} catch (Exception e) {
	...
	private void setup(JobConf conf, String targetPath, String backupPath)
		...
		if (dirNum == 0) {
			System.out.println("No dir need to be merged. Exiting...");
		} else {
			// set up a job
	...
	public int run(String[] args) throws Exception {
		try {
			...
			if (conf.getNumMapTasks() > 2) {
				System.out.println("Max running threads: " + conf.getNumMapTasks());
			}
	...
	private void printUsage() {
		System.out.println("Usage: YTHT [ -p dir_pattern ] [ -m map_num ] dir backup_dir ");
	}
