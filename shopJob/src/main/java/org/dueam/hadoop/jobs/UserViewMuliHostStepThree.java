 ...
 * Date: 11-1-4 下午1:14
 */
public class UserViewMuliHostStepThree extends Configured implements Tool, Reducer<Text, Text, Text, Text>, Mapper<Object, Text, Text, Text> {
    ...
    protected String[] getInputPath(String[] args) {
        ...
        String[] input = new String[]{"/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_two/" + args[0]};
        if (args.length > 1 && "false".equals(args[1])) {
            input = new String[]{"/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_two_all/" + args[0]};
        }
    ...
    protected String getOutputPath(String[] args) {
        if (args.length > 1 && "false".equals(args[1])) {
            return "/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_three_all/" + args[0];
        }
        return "/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_three/" + args[0];
    }
...
//        }
        ...
        if(dataList.size() > 1){
            for(int i=0;i<dataList.size() - 1;i++){
                for(String[] first : dataList.get(i)){
                    for(int k=i+1;k<dataList.size();k++){
                         for(String[] second : dataList.get(k)){
                               if(isSameTime(first[1],second[1])){
                                  if(sampleCount ++ > 100)  return;
                                  output.collect(key,Utils.mergeKey(first[1],first[2],first[3],first[4]));
                                  output.collect(key,Utils.mergeKey(second[1],second[2],second[3],second[4]));
                               }
                         }
                    }
                }
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(),
                new UserViewMuliHostStepThree(), args);
        System.exit(res);
    }
