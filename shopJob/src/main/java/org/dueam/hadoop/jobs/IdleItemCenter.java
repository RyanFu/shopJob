 ...
 * 闲置商品中心
 */
public class IdleItemCenter extends SimpleMapReduce {
    @Override
    ...
    protected String getOutputPath(String[] args) {
        return "/group/tbdev/xiaodu/suoni/idle_item_center/" + args[0];
    ...
    protected void doWork(String line, OutputCollector<Text, LongWritable> output) throws IOException {
        ...
        String features = _allCols[AuctionAuctions.FEATURES];

        String idle = Utils.getValue(features,"idle","0") ;
        //只统计新闲置的商品
        if(!"1".equals(idle)) {
            return;
        }
