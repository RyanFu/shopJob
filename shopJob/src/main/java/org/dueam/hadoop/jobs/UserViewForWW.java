 ...
 */
public class UserViewForWW extends Configured implements Tool, Reducer<Text, Text, Text, Text>, Mapper<Object, Text, Text, Text> {
    ...
    protected String getOutputPath(String[] args) {
        return "/group/tbdev/xiaodu/suoni/user_view_for_ww/" + args[0];
    }
    ...
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
        ...
        output.collect(key, new Text(ipSet.size()+"\t"+count));
    ...
    protected void doWork(String line, OutputCollector<Text, Text> output) throws IOException {
        ...
        output.collect(new Text("m"+"\t"+time.substring(0,12)+"00"), new Text(mid));
        output.collect(new Text("tm"+"\t"+time.substring(0,11)+"000"), new Text(mid));
        output.collect(new Text("h"+"\t"+time.substring(0,10)+"0000"), new Text(mid));
