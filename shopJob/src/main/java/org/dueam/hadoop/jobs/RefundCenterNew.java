 ...
 */
public class RefundCenterNew extends MapReduce<Text, LongWritable> {


    @Override
    protected void setup(String[] args) {
        this.output = "/group/tbdev/xiaodu/suoni/refund_center_new/" + args[0];
    ...
    public void map(Object key, Text value, OutputCollector<Text, LongWritable> output, Reporter reporter) throws IOException {
        ...
        String newRefund = Utils.getValue(_allCols[TcRefundTrade.attributes], "newRefund", "0");
        if (!"1".equals(newRefund)) {
            return;
        }
        //今天创建的订单相关统计
