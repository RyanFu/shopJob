 ...
 */
public class IdleTradeCenter extends SimpleMapReduce {
    @Override
    ...
    protected String getOutputPath(String[] args) {
        return "/group/tbdev/xiaodu/suoni/idle_trade_center/" + args[0];
    ...
    protected void doWork(String line, OutputCollector<Text, LongWritable> output) throws IOException {
        ...
        String idle = Utils.getValue(attr,"idle","0");
        //只统计新闲置的商品
        if(!(("1".equals(idle) || "2".equals(idle)))) {
           return;
        }

        //处理退款相关
        if (TcBizOrder.isDetail(_allCols)) {
            long totalFee = TcBizOrder.getTotalFee(_allCols);
            //退款引起的交易关闭
            if ("4".equals(payStatus)) {
                output.collect(Utils.mergeKey("refund", "close_num"), ONE);
                output.collect(Utils.mergeKey("refund", "close_sum"), new LongWritable(totalFee));
                output.collect(Utils.mergeKey("refund", "total_num"), ONE);
                output.collect(Utils.mergeKey("refund", "total_sum"), new LongWritable(totalFee));
            }
            if ("6".equals(payStatus)) {
                output.collect(Utils.mergeKey("refund", "success_num"), ONE);
                output.collect(Utils.mergeKey("refund", "success_sum"), new LongWritable(totalFee));
                output.collect(Utils.mergeKey("refund", "total_num"), ONE);  //所有结束交易的笔数
                output.collect(Utils.mergeKey("refund", "total_sum"), new LongWritable(totalFee)); //所有结束交易的金额
            }
            //是否属于退款完成状态（发生过退款）
            if ("5".equals(_allCols[TcBizOrder.REFUND_STATUS])) {
                output.collect(Utils.mergeKey("refund", "refund_num"), ONE);  //所有结束交易的笔数
                output.collect(Utils.mergeKey("refund", "refund_sum"), new LongWritable(totalFee)); //所有结束交易的金额
            }
        }
        ...
        if (Utils.isSameDay(gmtCreated, queryDate) && TcBizOrder.isDetail(_allCols) && TcBizOrder.isEffective(_allCols)) {
            output.collect(Utils.mergeKey("gmv","num"), ONE);
        }

        if (Utils.isSameDay(payTime, queryDate) && TcBizOrder.isDetail(_allCols) && TcBizOrder.isEffective(_allCols)) {
            output.collect(Utils.mergeKey("paid","num"), ONE);
        }

        //group by buyer  as gmv
        if (Utils.isSameDay(gmtCreated, queryDate) && TcBizOrder.isDetail(_allCols)) {
            ...
            output.collect(Utils.mergeKey("gmv_buyer", _allCols[TcBizOrder.BUYER_ID]), totalFee);
        }
