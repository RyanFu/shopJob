 ...
 * Date: 11-1-4 下午1:14
 */
public class UserViewMuliHostStepOne extends Configured implements Tool, Reducer<Text, Text, Text, Text>, Mapper<Object, Text, Text, Text> {
    ...
    protected String getOutputPath(String[] args) {
        if(args.length > 1 && "false".equals(args[1])){
            return "/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_one_all/" + args[0];
        }
        return "/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_one/" + args[0];
    }
    ...
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
        //判断是否单机逻辑 ： MID相同或者IP相同
        Set<String> ipSet = new HashSet<String>();
        Set<String> midSet = new HashSet<String>();
        List<Text> _temp = new ArrayList<Text>();
        ...
        while (values.hasNext()) {
            ...
            String mid = tokens[1];
            String ip = tokens[2];
            if (ipSet.isEmpty() && midSet.isEmpty()) {
                midSet.add(mid);
                ipSet.add(ip);
            } else {
                if (ipSet.contains(ip) || midSet.contains(mid)) {
                    ipSet.add(ip);
                    midSet.add(mid);
                } else {
                    muliHost = true;
    ...
    protected void doWork(String line, OutputCollector<Text, Text> output) throws IOException {
        ...
        boolean filterKeyPage = true;
        if (this.inputArgs.length > 1 && "false".endsWith(this.inputArgs[1])) {
            filterKeyPage = false;
        }
        if (!isKeyPage(url) && filterKeyPage) {
            return;
        }
        ...
        if (StringUtils.isEmpty(ip) || StringUtils.isEmpty(mid)) {
            return;
        }
        output.collect(new Text(userId), Utils.mergeKey(_allCols[Atpanel.time], mid, ip, url));
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(),
                new UserViewMuliHostStepOne(), args);
        System.exit(res);
    }
