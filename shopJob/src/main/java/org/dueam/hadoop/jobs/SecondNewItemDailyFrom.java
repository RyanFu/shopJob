 ...
 * @author suoni
 */
public class SecondNewItemDailyFrom extends Configured implements Tool {
    public int run(String[] args) throws Exception {
        ...
        JobConf conf = new JobConf(getConf(), SecondNewItemDailyFrom.class);
        conf.setJobName("SecondNewItemDailyFrom-" + System.currentTimeMillis());

        String date = args[0];
        FileSystem fs = FileSystem.get(conf);
        if (fs.exists(TaobaoPath.getOutput("second_new_item_daily_from", date))) {
            System.out.println("ERROR: You has finish this job at this day :  "
                    + date + " [ "
                    + TaobaoPath.getOutput("second_new_item_daily_from", date) + " ] ");
            return -1;
        }
        ...
        FileOutputFormat.setOutputPath(conf, TaobaoPath.getOutput(
                "second_new_item_daily_from", date));
    ...
    public static class MapClass extends MapReduceBase implements
        ...
        public void map(LongWritable key, Text value,
            ...
            if (!"6".equals(_allCols[AuctionAuctions.STUFF_STATUS])) {
                return;
            }
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new SecondNewItemDailyFrom(),
                args);
        System.exit(res);
    }
}
