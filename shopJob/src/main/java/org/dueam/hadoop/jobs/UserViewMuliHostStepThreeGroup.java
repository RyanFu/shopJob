 ...
 * Date: 11-1-4 下午1:14
 */
public class UserViewMuliHostStepThreeGroup extends Configured implements Tool, Reducer<Text, Text, Text, Text>, Mapper<Object, Text, Text, Text> {
    ...
    protected String getOutputPath(String[] args) {
        if (args.length > 1 && "false".equals(args[1])) {
            return "/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_three_group_all/" + args[0];
        }
        return "/group/tbdev/xiaodu/suoni/user_view_muli_host_setp_three_group/" + args[0];
    }
    ...
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
        ...
        if (count > 10000) {
            ...
            output.collect(key, Utils.mergeKey(String.valueOf(ipSet.size()), StringUtils.join(ipSet, '|')));
            return;
        }
...
//        }
        ...
        output.collect(key, Utils.mergeKey(String.valueOf(indexList.size()), StringUtils.join(ipSet, '|')));
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(),
                new UserViewMuliHostStepThreeGroup(), args);
        System.exit(res);
    }
