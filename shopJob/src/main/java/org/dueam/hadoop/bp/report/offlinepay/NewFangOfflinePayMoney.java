...
//===================================================================================
// File name: NewFangOfflinePayMoney.java
// Author: longque.zs
// Date: 2012-9-18 下午07:36:31 
// Description: 	 
// 		无
// Function List: 	 
// 		1. 无
...
/**
 * 线下支付-房产新房类目pos刷卡金额(包括新房垂直市场(50026396)、sell房产新房(50023944))
 * @author longque.zs
 * @version 1.0
 **/

public class NewFangOfflinePayMoney {
	
	private static char CTRL_A=(char)0x01; //分隔符

	/**
	 * 主要内容：
	 * 房产新房类目线上小定成交排行榜(每天)：卖家用户名和宝贝id分组  筛选内容：卖家用户名、商品id、成交笔数、支付宝成交金额
	 * 房产新房类目线下POS大定成交排行榜(每天)：卖家用户名和宝贝id分组  筛选内容：卖家用户名、商品id、成交笔数、支付宝成交金额
	 * 房产新房类目2012年线上小定成交排行榜(每年)：卖家用户名分组  筛选内容：卖家用户名、成交笔数、支付宝成交金额
	 * 房产新房类目2012年线下POS大定成交排行榜(每年)：卖家用户名分组  筛选内容：卖家用户名、成交笔数、支付宝成交金额
	 * @author longque.zs
	 * @param args
	 * @throws IOException
	 */
	public static void main(String[] args)throws IOException{
//		String input = "D:\\usr\\20121027"; // 文件路径
		String mainName="线下支付-房产新房"; //报表主名称
		...
		if (lines!= null && lines.size() > 0) {
			for(String line:lines){
				...
				if(line.startsWith(smallOrder_Perday)){ //pos小定-每天(成交笔数和成交金额)
					if(_cols.length > 0){
						if(StringUtils.isEmpty(preUserNickSmall)||StringUtils.equals(preUserNickSmall,_cols[1])){
							...
							smallOrderPerdayTable.addCol(preUserNickSmall+"小定合计数目",preUserSmallNum+"");
				...
				}else if(line.startsWith(smallOrder2012)){ //pos小定-2012每年
				...
				}else if(line.startsWith(bigOrder2012)){ ////pos大定-2012每年
				...
				}else if(line.startsWith(smallOrder2013)){ //pos小定-2013每年
				...
				}else if(line.startsWith(bigOrder2013)){ //pos大定-2013每年
